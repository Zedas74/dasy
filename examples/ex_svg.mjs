import { dasy, html, svg } from '../dist/dasy.mjs';
	...
	dasy({ data, container, afterRefresh }, (_, root) => html`
		<div>
			<input type="range" min="0" max="100" value="${
				root.with('.chart', $ => $.value)}" onInput="${e => root.set('.chart.value', e)}" />

			<div style="width: 180px; margin-top: 0.75rem">${svg`
				...
				<svg viewBox="0 0 120 120" width="180" height="180" role="img" aria-label="Pie chart">
					...
					<circle cx="60" cy="60" r="50" fill="none" stroke="#2563eb" stroke-width="18" 
						...
						root.with('.chart', $ => `${(clamp($.value) / 100) *pieCircumference} ${pieCircumference}`)}"></circle>
					<text x="60" y="60" text-anchor="middle" dominant-baseline="middle" style="fill: #111827">${
						root.with('.chart', $ => `${clamp($.value)}%`)}</text>
				</svg>
